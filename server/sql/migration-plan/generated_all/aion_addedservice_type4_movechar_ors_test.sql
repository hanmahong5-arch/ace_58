...
-- Source file: aion_AddedService_Type4_MoveChar_ORS_TEST.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_addedservice_type4_movechar_ors_test(_server_id INTEGER, _db_user_id TEXT, _db_passwd TEXT)
RETURNS INTEGER
LANGUAGE plpgsql AS $$
BEGIN
...
where (serviceType = 4 or serviceType = 14) and serviceFlag = 0 and status = 0 and toServer = _server
...
select idx, fromServer, fromCharacterId, fromCharacter, toCharacterId, toCharacter, fromUid, fromRace, serviceType, warehouse from AionAddedService (nolock)
...
fetch next from moveChar_cursor	into _idx, _from_server, _from_char_id, _from_char_name, _char_id, _char_name, _from_uid, _from_race, _service_type, _with_account_warehouse
...
begin
	...
	exec aion_AddedService_Type4_MoveChar_CheckOrgChar_ORS _from_server, _server, _db_user_id, _db_passwd, _from_char_id, _from_char_name, _preminum, _with_account_warehouse, 0, _val output
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_addedservice_type4_movechar_ors_test;
-- +goose StatementEnd
