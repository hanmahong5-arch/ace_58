...
-- Source file: aion_AvatarAddedService_Type4_MoveChar_Process_ORS.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_avataraddedservice_type4_movechar_process_ors(_db_user_id TEXT, _db_passwd TEXT, _from_server INTEGER, _from_char_id INTEGER, _from_char_name TEXT, _server INTEGER, _char_id INTEGER, _char_name TEXT, _premium INTEGER, _with_account_warehouse INTEGER, _mileage INTEGER, _char_id INTEGER, _ret INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql AS $$
BEGIN
...
begin
	...
	update user_data 
		...
		set delete_date = _delete_date, 
			...
			delete_type	= 10000	-- 某腐磐 捞傈栏肺 牢茄 昏力
...
_sql := 'SELECT user_id, _account_id = account_id '
			...
			+ '''select user_id, account_id, race INTO _from_char_name_real from ' + _database_name + '.user_data with (nolock) where char_id = ' + cast(_from_char_id as nvarchar) + ''') '
...
-- Normal 酒捞袍 沥焊 捞傈 (弊成 捞傈)

-- Warehouse Type 0 : 牢亥 & 厘馒

-- Warehouse Type 1 : 俺牢 芒绊

-- 6,7 : 拌沥 芒绊



-- Item Table 捞傈 (User 家蜡)
...
if _with_account_warehouse > 0		-- 拌沥芒绊 捞傈 炼扒牢 版快父 捞傈
...
begin
	...
	begin
		...
		_tmp_char_id := _account_id				-- 泅犁 酒捞袍 何啊 抛捞喉吝 user_item_option 父 拌沥芒绊牢 版快 account_id甫 荤侩
	...
	if _tmp_warehouse = 4		-- vendor 殿废 酒捞袍
...
-- inter buddy old character 涝仿 角菩
...
_sql := 'INSERT INTO user_stat ('
			...
			+ ' MagicalLeft, baseMagicalLeft, baseMagicalRight, MagicalRight, mpHealSkillBoost, baseMpHealSkillBoost '
...
_sql := 'Insert Into user_instance ('
			...
			+ 'char_id, world_id, instance_id, reentrance_time, server_id, count_variate, kina_increase, item_increase'

			+ ') '

			+ ' select '

			+ '' + cast(_char_id as nvarchar) + ', world_id, instance_id, reentrance_time, server_id, count_variate, kina_increase, item_increase'
...
_sql := 'INSERT INTO user_data_ext ('
			...
			+ ' char_id, creativity_point, usecp_resetcount, next_usecp_resetcount_dec_time, familiar_func_expireTime, familiar_energy, familiar_energy_autocharge, familiar_func_autocharge'

			+ ') '

			+ ' select '

			+ ' ' + cast(_char_id as nvarchar) + ', creativity_point, usecp_resetcount, next_usecp_resetcount_dec_time, familiar_func_expireTime, familiar_energy, familiar_energy_autocharge, familiar_func_autocharge'
...
begin
	...
	-- 快急 houseobject狼 id 啊 磊悼刘啊啊 酒聪扼 id甫 备茄促.  捞 何盒篮 荐沥窍扁肺 窍看栏骨肺 瞒饶 昏力秦具 茄促.

	-- 酒贰 _tmp_house_object_new_id甫 identity肺 备窍绰 巴篮 林籍秦瘤 鞘夸
...
declare _avatar_server_type	tinyint

_avatar_server_type := case when _server >= 70 then 1 else 0 end



declare _avatar_race		tinyint

_avatar_race := (select race from user_data where char_id = _char_id)



update	user_data

set		lev = lev + c.charLevAdjust,

		world = c.startWorld,

		xlocation = c.startXLoc,

		ylocation = c.startYLoc,

		zlocation = c.startZLoc,

		dir = c.startDir,

		resurrect_world = c.resurrectWorld,

		resurrect_xlocation = c.resurrectXLoc,

		resurrect_ylocation = c.resurrectYLoc,

		resurrect_zlocation = c.resurrectZLoc

from	user_data u

join	AvatarAddedService_Config c ON c.serverType = _avatar_server_type and c.race = _avatar_race
...
select	_char_id, questId, 1, 0, 0

from	AvatarAddedService_Quest

where	isQuestFinished = 0 and serverType = _avatar_server_type and race = _avatar_race
...
insert	user_finished_quest

select	_char_id, questId, 1, 0, datediff(s, '1970-01-01 09:00', NOW()), 0, 0

from	AvatarAddedService_Quest

where	isQuestFinished = 1 and serverType = _avatar_server_type and race = _avatar_race
...
select	_char_id, titleId, 1, 0

from	AvatarAddedService_Title

where	serverType = _avatar_server_type and race = _avatar_race
...
SELECT	_char_id, a.itemNameId, -1, a.itemCount, 0, 0, NOW(), NOW(), '', 0, 0, 0, 0, 0, 0, 0, 0, _server

from	AvatarAddedService_Item a

where	serverType = _avatar_server_type and _mileage between pointFrom and pointTo
...
-- Org Server狼 扁鸥 侩拌辑滚捞傈 包访 贸府

_sql := 'select * from '

			+ 'openrowset(''SQLOLEDB'', ' + _db_info + ','

			+ '''EXEC ' + _database_name + '.aion_AvatarAddedService_ClearUserData ' + cast(_from_char_id as nvarchar) + ''') '
...
if _with_account_warehouse > 0		-- 拌沥芒绊 捞傈 炼扒牢 版快父 昏力
...
begin
	...
	if _with_account_warehouse > 0		-- 拌沥芒绊 捞傈 炼扒牢 版快父 昏力, 沥惑利栏肺 费归捞 塞惦..
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_avataraddedservice_type4_movechar_process_ors;
-- +goose StatementEnd
