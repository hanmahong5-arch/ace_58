...
-- Source file: aion_MoveToLegacyAllUserData_OldUserOfLowlevel.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_movetolegacyalluserdata_olduseroflowlevel(_check_days INTEGER, _low_level INTEGER, _ncount INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql AS $$
BEGIN
...
RAISE NOTICE '%', _sql;
	...
	_update_time := GetUnixtimeWithUTCAdjust((NOW() AT TIME ZONE 'UTC'),0)



	UPDATE user_data SET delete_date = _update_time, delete_complete_date = _update_time where char_id in (select char_id from temp_user_id_object_to_del$)
	...
	_sql := 'SELECT TOP ' + CAST(_ncount AS nvarchar(50)) + ' char_id INTO temp_user_id_object$ FROM user_data WHERE delete_complete_date <> 0 AND (delete_complete_date + 3600*24*90 <' + CAST(_update_time AS nvarchar(20)) + ' OR ' + 'delete_complete_date = ' + CAST(_update_time AS nvarchar(20)) + ')'
RAISE NOTICE '%', _sql;
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_movetolegacyalluserdata_olduseroflowlevel;
-- +goose StatementEnd
