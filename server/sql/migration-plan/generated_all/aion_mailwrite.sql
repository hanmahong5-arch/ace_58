...
-- Source file: aion_MailWrite.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_mailwrite(_to_id INTEGER, _to_name TEXT, _from_id INTEGER, _from_name TEXT, _title TEXT, _content TEXT, _item_id BIGINT, _item_nameid INTEGER, _item_amount BIGINT, _money BIGINT, _warehouse INTEGER, _arrive_time INTEGER, _express_mail INTEGER)
RETURNS VOID
LANGUAGE plpgsql AS $$
BEGIN
if (_item_id <> 0)
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_mailwrite;
-- +goose StatementEnd
