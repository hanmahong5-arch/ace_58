...
-- Source file: aion_PutHouseScriptInstantToField.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_puthousescriptinstanttofield(_char_id INTEGER, _addr_id INTEGER)
RETURNS VOID
LANGUAGE plpgsql AS $$
BEGIN
...
BEGIN
	...
	DECLARE script_cursor CURSOR FOR (SELECT slot_id, script_size, script_data FROM house_instant_script WHERE char_id = _char_id);
	...
	BEGIN
		...
		EXEC aion_SetHouseFieldScript _addr_id, _slot_id, _script_size, _script_data;
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_puthousescriptinstanttofield;
-- +goose StatementEnd
