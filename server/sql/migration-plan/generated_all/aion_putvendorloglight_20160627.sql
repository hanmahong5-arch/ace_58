...
-- Source file: aion_PutVendorLogLight_20160627.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_putvendorloglight_20160627(_char_id INTEGER, _item_name_id INTEGER, _sold_price BIGINT, _sold_amount BIGINT, _remain_amount BIGINT, _sold_date INTEGER, _soul_bound INTEGER, _enchant_count INTEGER, _skin_name_id INTEGER, _stat_enchant0 INTEGER, _stat_enchant1 INTEGER, _stat_enchant2 INTEGER, _stat_enchant3 INTEGER, _stat_enchant4 INTEGER, _stat_enchant5 INTEGER, _option_count INTEGER, _dye_info INTEGER, _proc_tool_name_id INTEGER, _random_option INTEGER, _limit_enchant_count INTEGER, _reidentity_count INTEGER, _authorize_count INTEGER, _vanish_point INTEGER, _enchant_prob_addition INTEGER, _option_prob_addition INTEGER, _producer TEXT, _after_fee BIGINT, _after_tax BIGINT)
RETURNS VOID
LANGUAGE plpgsql AS $$
BEGIN
BEGIN
	...
	VALUES (_char_id, _item_name_id, _sold_price, _sold_amount, _remain_amount, _sold_date,
	 ...
	 _producer, _after_fee, _after_tax)
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_putvendorloglight_20160627;
-- +goose StatementEnd
