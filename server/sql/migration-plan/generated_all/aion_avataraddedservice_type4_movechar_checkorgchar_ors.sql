...
-- Source file: aion_AvatarAddedService_Type4_MoveChar_CheckOrgChar_ORS.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_avataraddedservice_type4_movechar_checkorgchar_ors(_from_server_id INTEGER, _server_id INTEGER, _db_user_id TEXT, _db_passwd TEXT, _char_id INTEGER, _char_name TEXT, _premium INTEGER, _with_account_warehouse INTEGER, _online INTEGER, _ret INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql AS $$
BEGIN
...
_sql := 'SELECT result INTO _val from openrowset (''SQLOLEDB'', ' + _db_info
			...
			+ ', ''' + _database_name + '.aion_AvatarAddedService_Type4_MoveChar_CheckOrgChar_local_ORS ' + cast(_from_server_id as nvarchar) 
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_avataraddedservice_type4_movechar_checkorgchar_ors;
-- +goose StatementEnd
