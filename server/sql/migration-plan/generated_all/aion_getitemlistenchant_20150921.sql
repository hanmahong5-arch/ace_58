...
-- Source file: aion_GetItemListEnchant_20150921.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION aion_getitemlistenchant_20150921(_char_id INTEGER)
RETURNS SETOF RECORD  -- TODO: replace with explicit RETURNS TABLE(col TYPE, ...)
LANGUAGE plpgsql AS $$
BEGIN
select	ID, COALESCE(soul_bound, 0), COALESCE(enchant_count, 0), COALESCE(skin_name_id, 0), COALESCE(stat_enchant_name0, 0), 
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS aion_getitemlistenchant_20150921;
-- +goose StatementEnd
