...
-- Source file: GM_UserItemDA_SrchMyItemByItemIDs.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION gm_useritemda_srchmyitembyitemids(_char_id INTEGER, _csv_item_ids TEXT)
...
set transaction isolation level read uncommitted
			...
			declare _query	nvarchar(4000)

			_query := '

			SELECT  i.id, i.char_id, i.name_id

					, slot_id, amount, slot, warehouse, create_date, update_date, producer, tid, expired_time, buy_amount, buy_duration, main_item_dbid, dynamic_property
			...
			WHERE	i.char_id = '+ CONVERT(varchar,_char_id) +'

			AND		i.id IN ('+ _csv_item_ids+ ')'
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS gm_useritemda_srchmyitembyitemids;
-- +goose StatementEnd
