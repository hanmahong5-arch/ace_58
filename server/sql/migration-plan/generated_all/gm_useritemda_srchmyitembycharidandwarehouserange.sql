...
-- Source file: GM_UserItemDA_SrchMyItemByCharIDandWarehouseRange.sql
-- Review TODOs before deploying.

-- +goose Up
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION gm_useritemda_srchmyitembycharidandwarehouserange(_char_id TEXT, _from_warehouse TEXT, _to_warehouse TEXT)
...
set transaction isolation level read uncommitted
			...
			SELECT	-- item
					...
					i.id, i.char_id, i.name_id, slot_id, amount, slot, warehouse

					, convert(nvarchar,create_date,21 ) AS create_date, convert(nvarchar,update_date,21 ) AS update_date, producer, tid, expired_time
					...
					, option_count, dye_info, proc_tool_nameid, obtain_skin_type, expire_skin_time, expire_dye_time, COALESCE(random_option,0) AS random_option, COALESCE(limit_enchant_count,0) AS limit_enchant_count
			...
			WHERE	i.char_id = _char_id and (warehouse between _from_warehouse and _to_warehouse);
END;
$$;
-- +goose StatementEnd
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION IF EXISTS gm_useritemda_srchmyitembycharidandwarehouserange;
-- +goose StatementEnd
